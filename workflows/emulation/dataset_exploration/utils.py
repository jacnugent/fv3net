...
def to_mappable(da, name):
    ...
    return mappable_var(merged_w_grid, name, **MAPPABLE_VAR_KWARGS)
...
def plot_var(ds, name, isel_kwargs=None, plot_kwargs=None, avg_dims=None):
    ...
    if avg_dims is not None:
        ...
        reduced = selected
    plot_cube(reduced, **plot_kwargs)
    
def plot_meridional(da, vkey, title="", ax=None, vmin=None, vmax=None):
    merged = da.to_dataset(name=vkey).merge(GRID)
    meridional = meridional_transect(mappable_var(merged, vkey, **MAPPABLE_VAR_KWARGS))
    if ax is None:
        fig, ax = plt.subplots()
        fig.set_dpi(120)
    ...
    meridional[vkey].plot.pcolormesh(
        x="lat", y=y, ax=ax, cmap="RdBu_r", vmin=vmin, vmax=vmax, yincrease=False,
